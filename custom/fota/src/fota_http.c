...
s32 HTTP_FotaMain(u8 contextId, u8* URL)
{
    ...
    ******************************************************************/
    ...
    Ql_memset(Fota_apn,0x00,sizeof(Fota_apn));	//add by christy
    Ql_strcpy(Fota_apn,APN);					//add by christy
...
s32 http_SendHttpGetHead( s32 socketId)
{
    ...
    //FOTA_DBG_PRINT("<--httpContext_p->hostport: ");	//add by christy
    //FOTA_DBG_PRINT(httpContext_p->hostport);			//add by christy
    ret =  Ql_SOC_Send(socketId, sendBuffer, Ql_strlen(sendBuffer));
...
cuntinuerecvdata:
    ...
    //FOTA_DBG_PRINT("<--want read socket datalength=-->\r\n");
